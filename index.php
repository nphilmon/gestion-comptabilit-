...
$assistantHeadline = 'Pilotage serein';
$assistantSummary = 'L\'activité est stable. Le logiciel ne détecte pas de risque majeur à court terme.';
$assistantFactors = [];
$assistantRecommendations = [];
$assistantTrendLabel = 'Pas assez de recul';
$assistantTrendValue = '0%';
$assistantTrendTone = 'info';
$moisCourantKey = sprintf('%d-%02d', $annee, (int) date('m'));
$moisPrecedentKey = sprintf('%d-%02d', (int) date('Y', strtotime($annee . '-' . date('m') . '-01 -1 month')), (int) date('m', strtotime($annee . '-' . date('m') . '-01 -1 month')));
$moisCourantStats = $statsMensuelles[$moisCourantKey] ?? ['recettes' => 0.0, 'depenses' => 0.0, 'solde' => 0.0];
$moisPrecedentStats = $statsMensuelles[$moisPrecedentKey] ?? ['recettes' => 0.0, 'depenses' => 0.0, 'solde' => 0.0];
$baseComparaison = $moisPrecedentStats['recettes'] > 0 ? $moisPrecedentStats['recettes'] : max(1, $moisCourantStats['recettes']);
$variationRecettes = $baseComparaison > 0
    ? round((($moisCourantStats['recettes'] - $moisPrecedentStats['recettes']) / $baseComparaison) * 100)
    : 0;
...
if ($statsCommerciales['factures_en_retard'] > 0) {
    $assistantScore -= min(28, $statsCommerciales['factures_en_retard'] * 8);
    $assistantFactors[] = [
        'label' => 'Impayés à relancer',
        'value' => $statsCommerciales['factures_en_retard'] . ' facture(s)',
        'tone' => 'danger',
    ];
    $assistantRecommendations[] = [
...
if ($statsCommerciales['en_attente_paiement'] > 0) {
    $assistantScore -= $statsCommerciales['en_attente_paiement'] > 5000 ? 16 : 8;
    $assistantFactors[] = [
        'label' => 'Encaissements attendus',
        'value' => formatMontant($statsCommerciales['en_attente_paiement']),
        'tone' => $statsCommerciales['en_attente_paiement'] > 5000 ? 'warning' : 'info',
    ];
}
...
if ($stats['benefice_net'] < 0) {
    ...
    $assistantRecommendations[] = [
        'text' => 'Le résultat est négatif. Réduis les dépenses ou accélère la facturation ce mois-ci.',
        'link' => BASE_URL . 'rapports.php?annee=' . $annee,
        'button' => 'Analyser',
    ];
}

if ($stats['plafond_ca'] > 0 && $stats['pct_plafond'] >= 90) {
    $assistantScore -= $stats['pct_plafond'] >= 98 ? 20 : 10;
    $assistantFactors[] = [
        'label' => 'Plafond micro',
        'value' => $stats['pct_plafond'] . '%',
        'tone' => $stats['pct_plafond'] >= 98 ? 'danger' : 'warning',
    ];
    $assistantRecommendations[] = [
        'text' => 'Le plafond micro est proche. Prépare un contrôle du CA et anticipe le changement de régime si nécessaire.',
...
if ($stockFaible > 0) {
    $assistantScore -= min(12, $stockFaible * 2);
    $assistantFactors[] = [
        'label' => 'Stock à surveiller',
        'value' => $stockFaible . ' produit(s)',
        'tone' => 'warning',
    ];
}

if ($variationRecettes >= 12) {
    $assistantScore += 6;
    $assistantTrendLabel = 'Dynamique mensuelle';
    $assistantTrendValue = '+' . $variationRecettes . '%';
    $assistantTrendTone = 'success';
    $assistantFactors[] = [
        'label' => 'Recettes du mois',
        'value' => '+' . $variationRecettes . '%',
        'tone' => 'success',
    ];
} elseif ($variationRecettes <= -12) {
    $assistantScore -= 10;
    $assistantTrendLabel = 'Ralentissement mensuel';
    $assistantTrendValue = $variationRecettes . '%';
    $assistantTrendTone = 'danger';
    $assistantRecommendations[] = [
        'text' => 'Les recettes ralentissent ce mois-ci. Pense à relancer les devis et à transformer les opportunités ouvertes.',
        'link' => BASE_URL . 'devis.php',
        'button' => 'Voir les devis',
    ];
} elseif ($moisCourantStats['recettes'] > 0 || $moisPrecedentStats['recettes'] > 0) {
    $assistantTrendLabel = 'Tendance mensuelle';
    $assistantTrendValue = ($variationRecettes > 0 ? '+' : '') . $variationRecettes . '%';
    $assistantTrendTone = abs($variationRecettes) >= 5 ? 'info' : 'primary';
}

$assistantScore = max(32, min(100, $assistantScore));

if ($assistantScore >= 85) {
    $assistantTone = 'success';
    $assistantHeadline = 'Pilotage serein';
    $assistantSummary = 'La situation est saine. Le logiciel voit surtout des optimisations, pas d\'urgence bloquante.';
} elseif ($assistantScore >= 65) {
    $assistantTone = 'warning';
    $assistantHeadline = 'Vigilance active';
    $assistantSummary = 'L\'activité tourne bien, mais quelques points méritent un suivi rapproché pour éviter une tension.';
} else {
    $assistantTone = 'danger';
    $assistantHeadline = 'Attention prioritaire';
    $assistantSummary = 'Le logiciel détecte plusieurs signaux faibles ou forts. Une action rapide est recommandée.';
}

if (empty($assistantFactors)) {
    $assistantFactors[] = [
...
if (empty($assistantRecommendations)) {
    $assistantRecommendations[] = [
        'text' => 'La base est propre. C\'est le bon moment pour préparer de nouveaux devis ou analyser les marges.',
        'link' => BASE_URL . 'devis.php?action=nouveau',
        'button' => 'Créer un devis',
    ];
}
